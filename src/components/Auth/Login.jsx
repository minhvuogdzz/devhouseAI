...
const Login = () => {
  ...
  return (
    <div className="min-h-screen bg-[#020617] flex items-center justify-center p-4 lg:p-8 relative overflow-hidden">
      {/* Background Effect */}
      <div className="absolute inset-0 z-0 bg-[radial-gradient(circle_at_center,rgba(56,189,248,0.15)_0%,transparent_50%)]"></div>
      <div className="absolute inset-0 z-0 bg-[linear-gradient(to_right,rgba(56,189,248,0.03)_1px,transparent_1px),linear-gradient(to_bottom,rgba(56,189,248,0.03)_1px,transparent_1px)] bg-[size:40px_40px]"></div>

      {/* Container Chính (Bọc 2 cột) */}
      <div className="relative z-10 w-full max-w-6xl flex flex-col lg:flex-row items-center justify-between gap-10 lg:gap-20 mb-8">
        
        {/* === CỘT TRÁI: THÔNG TIN TRANG CHỦ === */}
        <div className="flex-1 flex flex-col justify-center text-center lg:text-left mt-8 lg:mt-0">
          <div className="inline-flex items-center justify-center lg:justify-start gap-2 mb-6">
            <span className="px-3 py-1 rounded-full bg-sky-500/10 border border-sky-500/30 text-sky-400 text-xs font-bold uppercase tracking-widest">
              Dev House Group Inc.
            </span>
          </div>
          
          <h1 className="text-4xl lg:text-6xl font-black text-white mb-4 leading-tight drop-shadow-lg tracking-tight">
            Đột phá tư duy cùng <br className="hidden lg:block" />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-purple-500">
              Dev House AI
            </span>
          </h1>
          
          <p className="text-slate-400 text-sm lg:text-base leading-relaxed mb-8 max-w-lg mx-auto lg:mx-0">
            Trợ lý thông minh độc quyền tích hợp sức mạnh của mô hình trí tuệ nhân tạo hiện đại nhất hiện nay. Hỗ trợ bạn giải quyết mọi tác vụ lập trình, toán học, xử lý tài liệu và phân tích hình ảnh chuyên sâu.
          </p>
          ...
          <div className="grid grid-cols-1 max-sm:hidden sm:grid-cols-2 gap-4 text-left max-w-lg mx-auto lg:mx-0">
            ...
            <div className="flex items-center gap-3 bg-white/5 p-3.5 rounded-xl border border-white/10 hover:bg-white/10 transition-colors">
              <div className="w-8 h-8 rounded-lg bg-purple-500/20 text-purple-400 flex items-center justify-center text-lg">🖼️</div>
              <span className="text-sm font-medium text-slate-200">Thị giác máy tính</span>
            </div>
            <div className="flex items-center gap-3 bg-white/5 p-3.5 rounded-xl border border-white/10 hover:bg-white/10 transition-colors">
              <div className="w-8 h-8 rounded-lg bg-green-500/20 text-green-400 flex items-center justify-center text-lg">🎙️</div>
              <span className="text-sm font-medium text-slate-200">Nhận diện giọng nói</span>
            </div>
            <div className="flex items-center gap-3 bg-white/5 p-3.5 rounded-xl border border-white/10 hover:bg-white/10 transition-colors">
              <div className="w-8 h-8 rounded-lg bg-orange-500/20 text-orange-400 flex items-center justify-center text-lg">📄</div>
              <span className="text-sm font-medium text-slate-200">Xử lý tài liệu (PDF)</span>
            </div>
          </div>
        </div>

        {/* === CỘT PHẢI: FORM ĐĂNG NHẬP === */}
        <div className="w-full max-w-md bg-slate-900/60 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl flex flex-col shrink-0">
          <div className="flex flex-col items-center text-center mb-8">
              ...
              <h2 className="text-2xl font-bold text-white mb-2">{isLoginMode ? 'Chào mừng quay trở lại!' : 'Tạo tài khoản mới'}</h2>
              <p className="text-sm text-slate-400">{isLoginMode ? 'Đăng nhập để tiếp tục cuộc trò chuyện.' : 'Bắt đầu hành trình sáng tạo của bạn.'}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mb-6">
              ...
              <button 
                  type="submit" 
                  disabled={loading}
                  className="w-full bg-sky-500 hover:bg-sky-400 text-white font-bold py-3 px-4 rounded-xl transition-all hover:scale-[1.02] active:scale-95 shadow-[0_0_15px_rgba(14,165,233,0.3)] disabled:opacity-50 flex justify-center items-center"
              >
                  {loading ? (
      ...
      <div className="absolute bottom-4 left-0 w-full text-center z-10 pointer-events-none">
        <p className="text-[11px] lg:text-xs text-slate-500 font-medium tracking-wide">
          &copy; {new Date().getFullYear()} Dev House Group Inc. All rights reserved.
        </p>
      </div>
